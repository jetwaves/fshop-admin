...
var appID = 'wx81465e521747011f';
var appSecret = 'e3b8e0c47f91b4df0271c9e1fad85686';
...
var aSendRes = new Array();

// 获取token
function bulkSend(msg){
	var fullUrl = baseUrl + getTokenAction; 	console.log(' full URL = ' + fullUrl);
	https.get(baseUrl + getTokenAction, function(res){
...
// 循环群发消息
function sendMessageToGroup(msg, nextOpenId ){
	var getFansListURL = baseUrl + getFansListAction;
	getFansListURL = getFansListURL + appToken;
	...
	https.get(getFansListURL, function(res){
		res.on('data', function(body) {
			...
			if (next_openid != '') {
				// 拉取下一组用户并发送消息
				sendMessageToGroup(msg, next_openid);
			} 
			if ( count == 0) {
...
// 发送消息给一个用户
function sendMessageToOpenId(msg, openId){
	if (openId == '') { return; };
	var optSendSingleMsg = {
		hostname: 'api.weixin.qq.com',
		port: 443,
		path: sendServiceMsgAction + appToken,
		method: 'POST',
	};
	var msgBody = 	{
					    "touser": openId, 
	...
	var req = https.request(optSendSingleMsg, function(res) {
		res.on('data', function(body) {
			...
			if (parsedbody.errcode == 0 || parsedbody.errmsg == "ok") {
				console.log('	sendMessageToOpenId' + openId + ': Success: ╰(*°▽°*)╯  text message sent');
			} else {
				console.log('	sendMessageToOpenId' + openId + ': fail:   	(┯_┯)		errmsg:' + parsedbody.errmsg);
			}
			aSendRes.push(parsedbody);
		});
	});
	req.write(postData);
...
function sendAction(req, res){
	...
	setTimeout(function(){
		...
		for(idx in aSendRes){
			...
			if (statsRes[item.errcode] == null || statsRes[item.errcode] == undefined) {
				 statsRes[item.errcode] = { code: item.errcode, msg: item.errmsg, count: 0};
			} else{
	...
	// _infoJump(res, '发送成功，去看看自个手机吧' , '/messages/send');
}
